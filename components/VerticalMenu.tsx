		...
		<>
			{image && <>
				{item.url && !item.isActive ?
					<Link href={item.url}>
						<div className={clsx('vertical-menu__link img-link', isRootElem ? 'is-root' : 'is-child')}>
							{imageElem}
						</div>
					</Link>
					: imageElem}
			...
			{item.url && !item.isActive
				? <>
					<Link href={item.url}>
						<div className={clsx('vertical-menu__link title', isRootElem ? 'is-root' : 'is-child')} itemProp='url'>
							{isRootElem
